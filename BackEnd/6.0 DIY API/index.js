...
a=jokes.filter((joke)=>joke.jokeType===type)
});
res.json(a)
...
app.put('/jokes/:id',((req,res)=>{
  ...
  const toReplace=jokes.findIndex((joke)=>joke.id===id)
  jokes[toReplace]=replace
  res.json(replace)
}))
//6. PATCH a joke
...
const toReplaceAt=jokes.findIndex((joke)=>joke.id===id)
jokes[toReplaceAt]=newJoke
res.json(jokes[toReplaceAt]);
...
if(toReplaceAt > -1)
{
  jokes.splice(toReplaceAt,1)
  res.sendStatus(200)
}
else{
  res.status(400).json({error:`Joke With id:${id} not found. No Joke were deleted`})
}
  ...
  if(req.query.key===masterKey)
    {
      ...
      res.status(400).json({error:`Not authorized to delete all jokes`})
    } 
})
